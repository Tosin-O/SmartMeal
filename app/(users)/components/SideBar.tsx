...
export default function Sidebar({ isOpen, onClose }: SidebarProps) {
  ...
  return (
    <aside className={`fixed inset-y-0 left-0 z-50 w-64 bg-white dark:bg-[#111111] border-r border-gray-200 dark:border-[#2A2A2A] flex flex-col pt-6 pb-4 overflow-y-auto transition-transform duration-300 ease-in-out lg:translate-x-0 ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}> 
      ...
      <button 
        onClick={onClose}
        className="absolute top-4 right-4 p-2 text-gray-400 hover:text-gray-900 dark:hover:text-white lg:hidden"
      >
      ...
      <nav className="px-4 space-y-1 mb-auto">
        {navLinks.map((link) => {
          ...
          return (
            <Link key={link.name} href={link.href} className={`flex items-center gap-3 px-4 py-3 text-sm font-semibold rounded-xl transition-colors ${isActive ? 'bg-green-50 dark:bg-[#13251A] text-[#1CD05D]' : 'text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-[#1A1A1A] hover:text-gray-900 dark:hover:text-white'}`}>
      ...
      <div className="px-4 mt-8">
        <div className="p-5 border rounded-2xl bg-gray-50 dark:bg-[#0A0A0A] border-gray-200 dark:border-[#2A2A2A] shadow-sm relative overflow-hidden group transition-all hover:border-[#1CD05D]/30">
            ...
            <>
              <div className="flex items-center justify-between mb-4">
                <div className="flex flex-col">
                  ...
                  <p className="text-[9px] text-gray-500 font-bold uppercase mt-0.5">{activePlanCount} Plans Created</p>
                </div>
                {isOverBudget ? (
                  <span className="px-2 py-0.5 text-[9px] font-bold text-red-600 dark:text-red-500 bg-red-100 dark:bg-red-950/30 border border-red-200 dark:border-red-900/50 rounded uppercase tracking-wider">Over</span>
                ) : (
                  <span className="px-2 py-0.5 text-[9px] font-bold text-[#1CD05D] bg-green-100 dark:bg-[#13251A] border border-green-200 dark:border-[#1CD05D]/20 rounded uppercase tracking-wider">On Track</span>
                )}
              </div>
              
              <div className="flex items-baseline gap-1 mb-2">
                <span className="text-xl font-bold text-gray-900 dark:text-white">₦{spentAmount.toLocaleString()}</span>
                <span className="text-[10px] text-gray-500">/ ₦{budgetLimit.toLocaleString()}</span>
              </div>
              
              <div className="w-full h-1.5 mb-4 bg-gray-200 dark:bg-[#1A1A1A] rounded-full overflow-hidden">
                <div 
                  className={`h-full rounded-full transition-all duration-1000 ${isOverBudget ? 'bg-red-500' : 'bg-[#1CD05D] dark:shadow-[0_0_8px_rgba(28,208,93,0.4)]'}`} 
                  style={{ width: `${budgetPercentage}%` }}
                ></div>
              </div>
              
              <div className="flex justify-between items-center pt-2 border-t border-gray-200 dark:border-[#1A1A1A]">
                <span className="text-[10px] font-bold text-gray-500 uppercase tracking-tight">Remaining</span>
                <span className={`text-xs font-bold ${remaining < 0 ? 'text-red-500' : 'text-gray-900 dark:text-white'}`}>
      ...
      <div className="px-4 mt-4">
          <Link href="/dashboard/profile" className="flex items-center gap-3 px-4 py-3 text-xs font-bold text-gray-500 hover:text-gray-900 dark:hover:text-white transition-colors">
